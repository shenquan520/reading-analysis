...
def agent_ok(note, aid, index):
    ...
    ★ 第八轮 M1 修正（重要）：原版「左窗口 10 字内有对象词就否决」把**中文话题句**一起杀了——
      「**这段** 我概括得太宽了」：主体是**我**，「这段」只是话题（topic），不是施事（agent）。
      实测 5/5 误杀（「这段我…」「这个选项我…」这类极常见说法全不弹）。

    现行判据（在关键词所在子句内比较**谁离关键词更近**）：
      ① 取关键词所在子句的左半段
      ② 找最近的**对象词**；找不到 → 不否决
      ③ 找最近的「我」：
         · 「我」在对象词**之后**（更靠近关键词）→ 主体是用户 → **不否决**（话题句，M1）
         · 对象词更近 → 毛病属于文章 → **否决**（评价句）
    效果：评价句误报 0（含冗长说法），话题句漏报 0，既有 34 条正样本与 4 条防御用例全保持。
    """
    if strong_self(note):
        return True
    for k in (index[aid].get("kw_list") or keywords(index[aid].get("keywords", ""))):
        ...
        if left.rfind("我") > obj.start():
            return True          # 「我」比对象词更靠近关键词 → 主体是用户 → 保留
        return False             # 对象词更近 → 毛病属于文章 → 否决
    return True
    return True
